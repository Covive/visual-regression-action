// scripts/capture-with-baseurl.mjs
// Enhanced version that supports BASE_URL override for multi-dev testing
...
function buildTestUrl(originalUrl, baseUrl) {
    ...
    try {
        ...
        console.warn(`Failed to parse URL: ${originalUrl}, using original`);
        return originalUrl;
    }
}
...
// ---------- Main ----------
console.log(`[${PROJECT}] Starting capture...`);
if (BASE_URL) {
    console.log(`[${PROJECT}] 🔄 BASE_URL override: ${BASE_URL}`);
    console.log(`[${PROJECT}] All URLs will be rewritten to use this domain`);
}

ensureDirs(); // creates: baselines, artifacts/{current,diffs}, reports
...
let captureCount = 0;
const totalUrls = urls.length * SIZES.length;
...
for (const { name, url, waitFor = null, maskSelectors = [] } of urls) {
    for (const w of SIZES) {
        ...
        // Build the final URL (with BASE_URL override if provided)
        const finalUrl = buildTestUrl(url, BASE_URL);

        captureCount++;
        console.log(`[${PROJECT}] [${captureCount}/${totalUrls}] Navigating to: ${finalUrl}`);
        ...
        try {
            ...
            // Screenshot to artifacts/current/<slug>__<size>.png
            const out = pathFor('current', name, w);
            await page.screenshot({ path: out, fullPage: true });
            console.log(`[${PROJECT}] ✅ Captured ${out}`);
        } catch (error) {
            console.error(`[${PROJECT}] ❌ Failed to capture ${finalUrl}: ${error.message}`);
        } finally {
            await page.close();
        }
    }
...
await browser.close();
console.log(`[${PROJECT}] ✅ Capture complete! Total screenshots: ${captureCount}`);
